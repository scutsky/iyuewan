define("http://gm.hzjiulian.com/public/javascripts/dist/app/index/register.bak", [ "../../common/common", "../../common/validator", "../../common/utils", "../../common/alert-window", "../../common/title-window", "../../common/base-window" ], function(require) {
    ...
    var app = {
        ...
        _bind: function() {
            ...
            $('input[name="ch"]').on("change", function() {
                ...
                var val = $(this).val();
                $(".fields").hide();
                $('.fields[data-type="' + val + '"]').show();
            });
        },
        _checkPassword: function() {
            var that = this, $password = $("#password"), password = $password.val(), name = $password.attr("name"), valid = $password.valid(), error = {};
            if (valid) {
                valid = YM.utils.checkPwdStrength(password);
            } else {
                return valid;
            }
            if (!valid) {
                error[name] = "密码强度较弱，请试试字母、数字、符号的组合";
                this.validator.showErrors(error);
            }
            return valid;
        },
        _checkUserName: function(callback) {
            var that = this, $userName = $("#username"), name = $userName.attr("name"), value = $userName.val(), url = $userName.data("url"), error = {}, flag = false;
            if (!value) {
            ...
            $.get(url, {
                ...
                if (jsonData.state.code == 2e6) {
                    ...
                    flag = false;
                    error[name] = "" + jsonData.state.message;
                    that.validator.showErrors(error);
                }
                if (typeof callback === "function") {
                    callback(flag);
                }
        ...
        _initValidate: function() {
            var that = this;
            that.validator = $("#aspnetForm").validate({
                ignore: "",
                highlight: function(element, errorClass, validClass) {
                    ...
                    if ($(element).parent("div").find(".error-container").length === 0) {
                        $(element).parent("div").append('<span class="error-container"><i class="iconfont icon-alert m-r-5"></i></span>');
                    }
                },
                unhighlight: function(element, errorClass, validClass) {
                    ...
                    $(element).siblings(".error-container").remove();
                },
                errorPlacement: function(label, element) {
                    label.find(".icon-alert").remove();
                    $(element).parent("div").find(".error-container").append(label);
                },
                rules: {
                    "channel.username": {
                        required: true,
                ...
                messages: {
                    ...
                    note: {
                        required: "不能为空",
                        minlength: "最少输入10个字的申请说明",
                        maxlength: "已超过300字，请删除多余字数"
                    },
                    email: {
                        required: "请输入正确的邮箱地址",
                        mail: "请输入正确的邮箱地址",
                        rangelength: "请输入正确的邮箱地址",
                        remote: "该账号已存在，请输入其他账号"
                    },
                    password: {
                        required: "不能为空",
                        minlength: "不得少于6个字符",
                    ...
                    mobi: {
                        required: "不能为空",
                        number: "请输入正确的手机号",
                        exactlength: "请输入正确的手机号"
                    },
                    captcha: {
                        required: "不能为空",
                        exactlength: "验证码错误，重新输入",
                        remote: "验证码错误，重新输入"
                    },
                    company: {
                        required: "不能为空",
                ...
                submitHandler: function(form) {
                    that._checkUserName(function(isValid) {
                        if (isValid) {
        ...
        _submitForm: function(form) {
            var that = this;
            if (!that._checkPassword()) {
                return;
            }
            // ajax 提交
            var postData = $(form).serialize(), url = $(form).attr("action"), $submitBtn = $("#submitbtn");
            if ($submitBtn.data("sending")) return false;
            ...
            $.ajax({
                ...
                complete: function() {
                    ...
                    $submitBtn.val("提交").css("opacity", 1);
                },
                success: function(jsonData) {
                    if (jsonData.state.code === 2e6) {
                        window.location.href = "registerSuccess.htm"/*tpa=http://gm.hzjiulian.com/registerSuccess*/;
                    } else {
...
define("dist/common/validator", [], function(require) {
    require("http://gm.hzjiulian.com/public/javascripts/dist/app/index/dist/lib/jquery.validate.min");
...
!function(a) {
    "function" == typeof define && define.amd ? define("http://gm.hzjiulian.com/public/javascripts/dist/app/index/dist/lib/jquery.validate.min", [ "jquery" ], a) : a(jQuery);
}(function(a) {
    a.extend(a.fn, {
        validate: function(b) {
...
define("/public/javascripts/dist/app/index/register", [ "../../common/common", "../../common/validator", "../../common/utils", "../../common/alert-window", "../../common/title-window", "../../common/base-window" ], function(require) {
    require("../../common/common");
    require("../../common/validator");
    require("../../common/utils");
    var AlertWindow = require("../../common/alert-window");
    ...
    var app = {
        ...
        _bind: function() {
            ...
            YM.utils.bindMsgVerify();
            // 切换注册类型
            $('input[name="http://gm.hzjiulian.com/public/javascripts/dist/app/index/channelInfo.type"]').on("change", function() {
                // console.log($(this).val());
                var val = $(this).val();
                $(".fields").hide();
                $('.fields[data-type="' + val + '"]').show();
        ...
        _checkPassword: function() {
            ...
            if (!valid) {
                error[name] = '<i class="iconfont icon-alert m-r-5"></i> 密码强度较弱，请试试字母、数字、符号的组合';
                this.validator.showErrors(error);
            }
            return valid;
        },
        _checkUserName: function(callback) {
            ...
            $.get(url, {
                ...
                if (jsonData.state.code == 2e6) {
                    ...
                    flag = false;
                    error[name] = '<i class="iconfont icon-alert m-r-5"></i>' + jsonData.state.message;
                    that.validator.showErrors(error);
                }
                if (typeof callback === "function") {
                    callback(flag);
        ...
        _initValidate: function() {
            var that = this;
            that.validator = $("#form").validate({
                ignore: ":not(:visible)",
                /* 建议重构，可以去掉每个 message 前的 icon
                highlight: function( element, errorClass, validClass ) {
                    if ( element.type === "radio" ) {
                        this.findByName( element.name ).addClass( errorClass ).removeClass( validClass );
                    } else {
                ...
                rules: {
                    ...
                    "channel.confirmPassword": {
                        equalTo: "#password"
                    },
                    // 个人注册信息验证
                    "channelInfo.realname": {
                        required: true,
                        rangelength: [ 2, 20 ],
                        chinese: true
                    },
                    "channelInfo.identityCard": {
                        required: true,
                        idCard: true
                    },
                    "channelInfo.applySn": {
                    ...
                    "channelInfo.companyAddr": {
                        required: true,
                        minlength: 5,
                        maxlength: 50
                    },
                    "channelInfo.bizLicenseNo": {
                        required: true,
                        rangelength: [ 10, 20 ]
                    },
                    "channelInfo.bizRealname": {
                        required: true,
                        rangelength: [ 2, 20 ],
                        chinese: true
                    },
                    "channelInfo.bizIdentityCard": {
                        required: true,
                        idCard: true
                    },
                    "channelInfo.linkman": {
                    ...
                    "http://gm.hzjiulian.com/public/javascripts/dist/app/index/channelInfo.qq": {
                        required: true,
                        number: true,
                        rangelength: [ 5, 13 ]
                    },
                ...
                messages: {
                    ...
                    "channelInfo.realname": {
                        required: icon + "不能为空",
                        chinese: icon + "请输入真实的中文姓名",
                        rangelength: icon + "请输入真实的中文姓名",
                        maxlength: icon + "请输入真实的中文姓名"
                    },
                    "channelInfo.identityCard": {
                        required: icon + "不能为空",
                        idCard: icon + "请输入正确的号码",
                        maxlength: icon + "请输入正确的号码"
                    },
                    "channelInfo.applySn": {
                        required: icon + "不能为空",
                        minlength: icon + "最少输入10个字的申请说明",
                        maxlength: icon + "已超过300字，请删除多余字数"
                    },
                    "channelInfo.mobile": {
                        number: icon + "请输入正确的手机号",
                        exactlength: icon + "请输入正确的手机号"
                    },
                    "channelInfo.telphone": {
                        number: icon + "请输入正确的电话号码",
                        rangelength: icon + "请输入正确的电话号码"
                    },
                    // 公司注册信息验证
                    "channelInfo.companyName": {
                        required: icon + "不能为空",
                        minlength: icon + "公司名称必须大于5个字",
                        maxlength: icon + "公司名称长度不得大于30个字"
                    },
                    "channelInfo.companyAddr": {
                        required: icon + "不能为空",
                        minlength: icon + "公司地址必须大于5个字",
                        maxlength: icon + "公司地址长度不得大于50个字"
                    },
                    "channelInfo.bizLicenseNo": {
                        required: icon + "不能为空",
                        rangelength: icon + "请输入正确的营业执照编号",
                        maxlength: icon + "请输入正确的营业执照编号"
                    },
                    "channelInfo.bizRealname": {
                        required: icon + "不能为空",
                        chinese: icon + "请输入真实的中文姓名",
                    ...
                    "channelInfo.linkman": {
                        required: icon + "不能为空",
                        chinese: icon + "请输入真实的中文姓名",
                        rangelength: icon + "2~20个中文",
                        maxlength: icon + "2~20个中文"
                    },
                    "http://gm.hzjiulian.com/public/javascripts/dist/app/index/channelInfo.qq": {
                        required: icon + "不能为空",
                        number: icon + "请输入正确的号码",
                        rangelength: icon + "请输入正确的号码",
                        maxlength: icon + "请输入正确的号码"
                    },
                    "channelInfo.linkqq": {
                        required: icon + "不能为空",
            ...
            $("#checkCode").rules("add", {
                ...
                rangelength: [ 4, 4 ],
                messages: {
                    digits: '<i class="iconfont icon-alert m-r-5"></i>请输入正确的验证码',
                    rangelength: '<i class="iconfont icon-alert m-r-5"></i>请输入正确的验证码'
                }
            });
        },
        _submitForm: function(form) {
