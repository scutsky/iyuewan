...
namespace Spread.DAL
{
    public class ReportTemp
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.ReportTemp model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into ReportTemp(");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.ReportTemp model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ReportTemp set ");
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from ReportTemp ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,SumDate,ChannelID,ChannelName,GameID,GameName,RegisterValue,ConsumptionValue,Income,AddDate,UserID,UserName,Bak1,Bak2,Bak3,Bak4 ");
            strSql.Append(" FROM ReportTemp ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM ReportTemp ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from ReportTemp T ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" SELECT  SumDate, UserID, COUNT(*) AS num, SUM(RegisterValue) AS RegisterValue, SUM(Income) AS Income ");
            strSql.Append(" FROM  dbo.ReportTemp where 0=0 " + strWhere + "");
            strSql.Append(" GROUP BY SumDate, UserID ");
              return DbHelper.Query(strSql.ToString());
        }
