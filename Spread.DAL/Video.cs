...
namespace Spread.DAL
{
    public class Video
    {
		...
		/// 增加一条数据
		/// </summary>
		public void Add(Spread.Model.Video model)
		{
			StringBuilder strSql=new StringBuilder();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Video set " + strValue);
            strSql.Append(" where Id=" + Id);
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Spread.Model.Video model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新内容
		/// </summary>
        public void UpdateContent(Spread.Model.Video model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Video set ");
		...
		/// 得到一个对象实体
		/// </summary>
		public Spread.Model.Video GetModel(int Id)
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 Id,Title,Form,ClassId,ImgUrl,IsMsg,IsTop,IsRed,IsHot,IsSlide,IsLock,AddTime,Zhaiyao,GType,GFactory,GBrand,Code1,Code2,LanguageType from Video ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)};
			parameters[0].Value = Id;

			Spread.Model.Video model=new Spread.Model.Video();
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public Spread.Model.Video GetUpDownModel(int Id, bool UpOrDown)
		{
			StringBuilder strSql=new StringBuilder();
            if (UpOrDown)
            {
                strSql.Append("select top 1 * from [Video] WHERE Id > @Id order by Id asc ");
            }
            else { strSql.Append("select * from [Video] WHERE Id < @Id order by Id desc "); }
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)};
			parameters[0].Value = Id;
        ...
        public DataSet GetPageList(int pageSize, int currentPage, string strWhere, string filedOrder)
        {
            ...
            if (currentPage > 0)
            {
                ...
                strSql.Append("select top " + pageSize + " Id,Title,Form,Zhaiyao,ClassId,ImgUrl,IsMsg,IsTop,IsRed,IsHot,IsSlide,IsLock,AddTime,GType,GFactory,GBrand,Code1,Code2,LanguageType from Video");
                strSql.Append(" where Id not in(select top " + topNum + " Id from Video");
                if (strWhere.Trim() != "")
                {
                    strSql.Append(" where " + strWhere);
                }
