...
namespace Spread.DAL
{
    public class Means
    {
		...
		public bool Exists(int Id)
		{
			...
			strSql.Append("select count(1) from Means");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Means(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Means set " + strValue);
            strSql.Append(" where Id=" + Id);
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Means set ");
		...
		public void Delete(int Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Means ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)};
			parameters[0].Value = Id;
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 Id,Title,Form,ClassId,ImgUrl,IsMsg,IsTop,IsRed,IsHot,IsSlide,IsLock,AddTime,Zhaiyao,GType,GFactory,GBrand,Code1,Code2,LanguageType from Means ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)};
			parameters[0].Value = Id;

			Spread.Model.Means model=new Spread.Model.Means();
        ...
        public Spread.Model.Means GetUpDownModel(int Id, bool UpOrDown)
		{
            ...
            if (UpOrDown)
            {
                strSql.Append("select top 1 * from [Means] WHERE Id > @Id order by Id asc ");
            }
            else { strSql.Append("select * from [Means] WHERE Id < @Id order by Id desc "); }
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)};
			parameters[0].Value = Id;
        ...
        public DataSet GetPageList(int pageSize, int currentPage, string strWhere, string filedOrder)
        {
            ...
            if (currentPage > 0)
            {
                ...
                strSql.Append("select top " + pageSize + " Id,Title,Form,Zhaiyao,ClassId,ImgUrl,IsMsg,IsTop,IsRed,IsHot,IsSlide,IsLock,AddTime,GType,GFactory,GBrand,Code1,Code2,LanguageType from Means");
                strSql.Append(" where Id not in(select top " + topNum + " Id from Means");
                if (strWhere.Trim() != "")
                {
                    strSql.Append(" where " + strWhere);
                }
